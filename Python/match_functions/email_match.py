...
def write_connections(name_table_1, df1, sheet1, column1, name_table_2, df2, sheet2, column2, final_score, columntype_score_1, columntype_score_2):
    ...
    ws1 = wb1[sheet1]
    ws1.cell(row=1, column=width1 + 1).value = 'connection_e'
    ws1.cell(row=1, column=width1 + 2).value = 'score_e'

    wb2 = openpyxl.load_workbook(filename=name_table_2, read_only=False)
    ws2 = wb2[sheet2]
    ws2.cell(row=1, column=width2 + 1).value = 'connection_e'
    ws2.cell(row=1, column=width2 + 2).value = 'score_e'
