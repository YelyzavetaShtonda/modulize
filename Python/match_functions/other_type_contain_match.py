...
def find_connections(column1, column2, final_score):
    ...
    for i in range(max(len(column1), len(column2))):
        ...
        for j in range(len(column2)):
            if len(column1[i]) > 10 and len(column2[j]) > 10:
                if column1[i] != column2[j] and (column1[i] in column2[j] or column2[i] in column1[j]):  # check if one string is substring of another and they are not the same
...
def write_connections(name_table_1, df1, sheet1, column1, name_table_2, df2, sheet2, column2, final_score, columntype_score_1, columntype_score_2):
    ...
    ws1 = wb1[sheet1]
    ws1.cell(row=1, column=width1 + 1).value = 'connection_other_contain'
    ws1.cell(row=1, column=width1 + 2).value = 'score_other_contain'

    wb2 = openpyxl.load_workbook(filename=name_table_2, read_only=False)
    ws2 = wb2[sheet2]
    ws2.cell(row=1, column=width2 + 1).value = 'connection_other_contain'
    ws2.cell(row=1, column=width2 + 2).value = 'score_other_contain'
