...
def categorize_trx_id(df):
    ...
    for i in range(len(df.columns)):
        ...
        for j in range(len(df.index)):
            ...
            elif re.match(r"^[^+][0-9]{9,}", str(df.iloc[j][i])) and re.match(r"^[^:]+$",
                                                                              str(df.iloc[j][i])) and not re.search(
                r"[\s]", str(df.iloc[j][
                                 i])):  # if it has more than 10 digits and doesn`t have a + and : sign and does not have spaces
                list_of_result_for_each_cell.append(1)
                flag = 1
...
def calculate_final_output(results):
...
def add_data_to_main_df(df, res, filename, sheetname):
    ...
    new_filename = f'{filename}_{sheetname}_processed_file.csv'
    # new_rows.to_csv(new_filename, index=False)
    df.to_excel(f'{filename}_{sheetname}_processed_entire_file.xlsx', index=False)

    return df


# add_data_to_main_df(df, res)


def main(filename, sheetname):
    try:
        df = pd.read_excel(f'{filename}', sheet_name=sheetname)
    except ValueError:
        df = pd.read_csv(f'{filename}')
    # cat = pd.read_excel('Data type recognision.xlsx', sheet_name='Categorization')
    cat = categor_df
    ...
    res = calculate_final_output(result3)
    # print(res)
    final_data = add_data_to_main_df(df, res, filename, sheetname)

    return res

print(main('PSP_to_Airtable_1_copy.xlsx', 'Fibonatix'))